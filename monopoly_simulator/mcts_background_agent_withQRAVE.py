...
class Node:
    def __init__(self, state, parent=None, action=None):
        ...
        # RAVE statistics: For each action, store cumulative value and count
        self.rave_values = {}
        self.rave_counts = {}
    ...
    def best_child(self):
        ...
        for c in self.children:
            ...
            rave_val = self.get_rave_value_for_child(c.action)
    ...
    def get_rave_value_for_child(self, action):
        # Return the average rave value for given action from this node
...
def run_mcts(player, current_gameboard, allowable_moves, max_iterations=50):
    ...
    for _ in range(max_iterations):
        ...
        # For each node in path (except leaf), update RAVE stats for all actions that appeared later.
        update_rave(path, actions_taken, reward)
...
def update_rave(path, actions_taken, reward):
    """
    Update RAVE stats for each node in path with the actions that appeared after that node.
    For each node in the path, every action that appears downstream in actions_taken (excluding that node's own action)
    gets its RAVE stats updated.
    """
    # actions_taken is the full set of actions chosen in the simulation after we reached the leaf
    # For Q-RAVE, we update from each node in path: all actions in actions_taken (except possibly the action leading to that node)
    # Typically, we consider actions_taken from the root after it was selected.
    # Because we stored entire path and actions_taken includes the initial action at leaf,
    # we can consider all actions in actions_taken as "AMAF" actions for all nodes above.

    # We'll assume that the first node in path corresponds to before action actions_taken[0].
    # So for node i in path, we update with all actions in actions_taken[i+1:] (actions after that node).
