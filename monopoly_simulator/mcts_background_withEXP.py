...
def run_mcts(player, current_gameboard, allowable_moves, max_iterations=50):
    """
    Run MCTS with EXP3-based action selection.
    ...
    for _ in range(max_iterations):
        ...
        # BACKPROPAGATION
        # Update nodes along the path with the observed reward
        node_to_update = root
        for depth, (chosen_index, chosen_probability) in enumerate(zip(chosen_indices, chosen_probabilities)):
            # Update the weight of the chosen child
    ...
    # Choose the best action based on the highest average value
    # (value / visits). If visits are zero, treat as zero.
    # Alternatively, you might choose the action with the highest weight.
    best_child = max(
        root.children,
...
def handle_negative_cash_balance(player, current_gameboard):
    ...
    if mortgage_potentials and (max_sum + player.current_cash >= 0):
        sorted_potentials = sorted(mortgage_potentials, key=lambda x: x[1], reverse=True)
    ...
    if sale_potentials:
        sorted_sp = sorted(sale_potentials, key=lambda x: x[1], reverse=True)
        allowable = set(f"sell:{sp[0].name}" for sp in sorted_sp)
        chosen_action = run_mcts(player, current_gameboard, allowable, max_iterations=30)
        if chosen_action is not None:
    ...
    # Final option: sell all properties if needed
