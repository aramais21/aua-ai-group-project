...
GLOBAL_RAVE_PATH = "seasonal_rave_stats.pkl"


class GlobalRAVE:
    """
    A class to store and manage global RAVE statistics across multiple games,
    categorized by game phases: early, mid, and late.
    """

    def __init__(self):
        # Initialize separate RAVE dictionaries for each phase
        self.rave_values = {
            'early': {},
            'mid': {},
            'late': {}
    ...
    def update(self, actions, reward, phase):
        """
        Update RAVE statistics with actions taken and the reward received,
        categorized by game phase.
        :param actions: List of actions taken during the game.
        :param reward: Reward received at the end of the game.
        :param phase: Current game phase ('early', 'mid', 'late').
        """
        if phase not in self.rave_values:
            logger.warning(f"Unknown phase '{phase}'. Skipping RAVE update.")
            return

        logger.debug(f"Updating RAVE for phase '{phase}' with reward {reward}. Actions: {actions}")

        for action in actions:
            if action not in self.rave_values[phase]:
                self.rave_values[phase][action] = 0.0
                self.rave_counts[phase][action] = 0
            self.rave_values[phase][action] += reward
            self.rave_counts[phase][action] += 1

    def get_rave_value(self, action, phase):
        """
        Get the average RAVE value for a given action and phase.
        :param action: Action to retrieve the RAVE value for.
        :param phase: Current game phase ('early', 'mid', 'late').
        :return: Average RAVE value or a neutral guess if no data.
        """
        if (phase in self.rave_counts and
                action in self.rave_counts[phase] and
                self.rave_counts[phase][action] > 0):
            return self.rave_values[phase][action] / self.rave_counts[phase][action]
        else:
            # No RAVE data for this phase, return neutral (0.5 as a neutral guess)
            return 0.5
    ...
    def save(self, path=GLOBAL_RAVE_PATH):
        ...
        try:
            ...
            logger.info(f"Global RAVE statistics saved to {path}.")
        except Exception as e:
            logger.error(f"Failed to save Global RAVE statistics: {e}")

    def load(self, path=GLOBAL_RAVE_PATH):
        """
        Load the global RAVE statistics from a file.
        Ensure that 'early', 'mid', and 'late' keys are present.
        """
            ...
            try:
                with open(path, 'rb') as f:
                    ...
                    for phase in ['early', 'mid', 'late']:
                        if phase not in loaded_values:
                            loaded_values[phase] = {}
                            logger.info(f"Phase '{phase}' missing in RAVE values. Initializing empty dictionary.")
                        if phase not in loaded_counts:
                            loaded_counts[phase] = {}
                            logger.info(f"Phase '{phase}' missing in RAVE counts. Initializing empty dictionary.")

                    self.rave_values = loaded_values
                    self.rave_counts = loaded_counts
                logger.info(f"Global RAVE statistics loaded from {path}.")
            except Exception as e:
                logger.error(f"Failed to load Global RAVE statistics: {e}")
                # Initialize empty RAVE if loading fails
                self.rave_values = {'early': {}, 'mid': {}, 'late': {}}
                self.rave_counts = {'early': {}, 'mid': {}, 'late': {}}
        else:
...
class Node:
    ...
    def expand(self, actions):
        ...
        logger.debug(f"Expanded node with actions: {actions}")

    def is_leaf(self):
        return len(self.children) == 0

    def best_child(self, current_phase):
        """
        Choose child based on Q-RAVE combination, considering the current game phase.
        :param current_phase: Current game phase ('early', 'mid', 'late').
        """
        if self.visits == 0:
            chosen = random.choice(self.children)
            logger.debug(f"Selecting random child '{chosen.action}' as parent has zero visits.")
            return chosen
        ...
        for c in self.children:
            ...
            rave_val = self.get_rave_value_for_child(c.action, current_phase) + global_rave.get_rave_value(c.action,
                                                                                                           current_phase)
            combined = (1 - beta) * q_val + beta * rave_val
            logger.debug(f"Child '{c.action}': Q={q_val}, RAVE={rave_val}, Combined={combined}")
            if combined > best_value:
                best_value = combined
                best = c
        logger.debug(f"Best child selected: '{best.action}' with combined value {best_value}")
        return best

    def get_rave_value_for_child(self, action, phase):
        """
        Return the average rave value for given action from this node (local RAVE),
        considering the game phase.
        :param action: Action to retrieve the RAVE value for.
        :param phase: Current game phase ('early', 'mid', 'late').
        :return: Average RAVE value or a neutral guess if no data.
        """
...
def determine_game_phase(current_gameboard):
    """
    Determine the current game phase based on gameboard state.
    :param current_gameboard: The current state of the gameboard.
    :return: String indicating the game phase ('early', 'mid', 'late').
    """
    # Example criteria (you can adjust based on your game's logic)
    total_turns = current_gameboard.get('total_turns', 0)
    if total_turns < 20:
        phase = 'early'
    elif total_turns < 40:
        phase = 'mid'
    ...
    logger.debug(f"Determined game phase: {phase} based on total_turns: {total_turns}")
    return phase


def run_mcts(player, current_gameboard, allowable_moves, max_iterations=50):
    if not allowable_moves or len(allowable_moves) == 1:
        logger.debug("Allowable moves are empty or only one option available. Skipping MCTS.")
        return None
    ...
    phase = determine_game_phase(current_gameboard)
    ...
    for iteration in range(max_iterations):
        ...
        update_rave(path, actions_taken, reward, phase, global_update=True)

        logger.debug(f"Iteration {iteration + 1}/{max_iterations}: Action='{action}', Reward={reward}")

    # Choose best action by visit count
    best_child = max(root.children, key=lambda c: c.visits)
    logger.debug(f"Chosen action by MCTS: '{best_child.action}' with visits: {best_child.visits}")
...
def update_rave(path, actions_taken, reward, phase, global_update=False):
    """
    ...
    for i, node in enumerate(path):
        ...
        for a in actions_taken[i + 1:]:
            ...
            logger.debug(f"Updated local RAVE for node '{node.action}' with action '{a}' and reward {reward}")

    if global_update:
        # Update global RAVE stats once per simulation
        global_rave.update(actions_taken, reward, phase)
        logger.debug(f"Updated global RAVE for phase '{phase}' with reward {reward}.")
...
def make_buy_property_decision(player, current_gameboard, asset):
    ...
    if player.current_cash - asset.price >= current_gameboard['go_increment']:
        decision = True
                                                                                                  ...
                                                                                                  current_gameboard):
        decision = True
    logger.debug(f"Make buy property decision for '{asset.name}': {decision}")
    return decision
...
def make_bid(player, current_gameboard, asset, current_bid):
    ...
    if not possible_bids:
        logger.debug(f"No possible bids available for asset '{asset.name}'. Passing.")
        return 0
    ...
    if chosen_action is not None:
        if chosen_action == "0":
            ...
            try:
                bid_value = int(float(chosen_action))
                logger.debug(f"Chosen bid: {bid_value}")
                return bid_value
            except ValueError:
    ...
    if current_bid < asset.price:
        ...
        if new_bid < player.current_cash:
            logger.debug(f"Fallback bid based on heuristic: {new_bid}")
            return new_bid
        else:
            return 0
    elif current_bid < player.current_cash and agent_helper_functions.will_property_complete_set(player, asset,
                                                                                                 current_gameboard):
        new_bid = current_bid + (player.current_cash - current_bid) // 4
        logger.debug(f"Fallback bid based on monopoly completion: {new_bid}")
        return new_bid
    else:
        return 0
...
def handle_negative_cash_balance(player, current_gameboard):
    ...
    if mortgage_potentials and max_sum + player.current_cash >= 0:
        ...
        if len(sorted_potentials) > 1:
            ...
            if chosen_action is not None:
                if chosen_action.startswith("mortgage:"):
                    ...
                    player.agent._agent_memory['previous_action'] = "mortgage_property"
                    logger.debug(f"Chosen action: 'mortgage_property' on '{chosen_property}'")
                    return ("mortgage_property", params)
        ...
        for p in sorted_potentials:
            ...
            player.agent._agent_memory['previous_action'] = "mortgage_property"
            logger.debug(f"Fallback action: 'mortgage_property' on '{p[0].name}'")
            return ("mortgage_property", params)
    ...
    if sale_potentials:
        ...
        if len(sorted_sp) > 1:
            ...
            if chosen_action is not None:
                if chosen_action.startswith("sell:"):
                    ...
                    player.agent._agent_memory['previous_action'] = "sell_property"
                    logger.debug(f"Chosen action: 'sell_property' on '{chosen_property}'")
                    return ("sell_property", params)
        ...
        for p in sorted_sp:
            ...
            player.agent._agent_memory['previous_action'] = "sell_property"
            logger.debug(f"Fallback action: 'sell_property' on '{p[0].name}'")
            return ("sell_property", params)
        ...
        if sell_improvement_actions:
            if len(sell_improvement_actions) > 1:
                ...
                if chosen_action is not None:
                    ...
                    logger.debug(f"Chosen action: 'sell_house_hotel' on '{prop_name}'")
                ...
                if act == "sell_house":
                    ...
                    params['sell_hotel'] = True
                player.agent._agent_memory['previous_action'] = "sell_house_hotel"
                logger.debug(f"Fallback action: 'sell_house_hotel' on '{prop_name}'")
                return ("sell_house_hotel", params)
    ...
    if final_assets_list:
        ...
        for a in final_assets_list:
            ...
            player.agent._agent_memory['previous_action'] = "sell_property"
            logger.debug(f"Fallback action: 'sell_property' on '{a.name}'")
            return ("sell_property", params)
