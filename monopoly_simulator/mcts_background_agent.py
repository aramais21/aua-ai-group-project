...
def run_mcts(player, current_gameboard, allowable_moves, max_iterations=50):
    """
    Run a basic MCTS on the current decision point:
    - State: (player, current_gameboard)
    - Actions: allowable_moves
    We simulate random outcomes for each action as rollouts.
    This is a simplification. In a real scenario, we would do deep copies of the state
    ...
    # This is a simplified representation, just for demonstration.
    ...
    for _ in range(max_iterations):
        ...
        # EXPAND (not needed if already expanded at root, but if node has no children and more moves possible)
        # In this simplified scenario, we only expand from root. If needed, we could do so here.
        # For now, node is likely a leaf. We'll do a rollout.

        # SIMULATE (rollout)
        # We do a random rollout and assign a random reward based on a heuristic:
        # If action is something beneficial (like buy_property), we give a slightly higher random reward.
        # This is entirely a placeholder. In a full MCTS for Monopoly, you'd simulate a few moves ahead.
        action = node.action
